...
class Items extends CI_Controller
{
  ...
  public function lilt()
  {
    ...
    $data['range'] = $totalItems > 0 ? "Hiển thị " . ($start + 1) . "-" . ($start + count($data['allItems'])) . " of " . $totalItems : "";
  ...
  public function add()
  {
    ...
    $this->form_validation->set_rules('itemQuantity', 'Số lượng', ['required', 'trim', 'numeric'], ['required' => "required"]);
    $this->form_validation->set_rules('itemPrice', 'Đơn giá', ['required', 'trim', 'numeric'], ['required' => "required"]);
    $this->form_validation->set_rules(
      'itemCode',
      'Mã sản phẩm',
      ['required', 'trim', 'max_length[20]', 'is_unique[items.code]'],
      ['required' => "required", 'is_unique' => "Đã có một mặt hàng với mã này"]
    );
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      $desc = "Bổ sung {$itemQty} số lượng cho sản phẩm mới '{$itemName}' với đơn giá {$itemPrice} đến kho hàng";

      $insertedId ? $this->genmod->addevent("Tạo mới", $insertedId, $desc, "sản phẩm", $this->session->admin_id) : "";

      $this->db->trans_complete();

      $json = $this->db->trans_status() !== FALSE ?
        ['status' => 1, 'msg' => "Thêm sản phẩm thành công"]
        :
        ['status' => 0, 'msg' => "Lỗi máy chủ không mong muốn! Vui lòng liên hệ với quản trị viên để được trợ giúp."];
    } else {
      //return all error messages
      $json = $this->form_validation->error_array(); //get an array of all errors

      $json['msg'] = "Một hoặc nhiều trường bắt buộc trống hoặc không được điền chính xác";
      $json['status'] = 0;
    }
  ...
  public function updatestock()
  {
    ...
    $this->form_validation->set_rules('_iId', 'ID sản phẩm', ['required', 'trim', 'numeric'], ['required' => "required"]);
    $this->form_validation->set_rules('_upType', 'Kiểu cập nhật', ['required', 'trim', 'in_list[newStock,deficit]'], ['required' => "required"]);
    $this->form_validation->set_rules('qty', 'Số lượng', ['required', 'trim', 'numeric'], ['required' => "required"]);
    $this->form_validation->set_rules('desc', 'Cập nhật mô tả', ['required', 'trim'], ['required' => "required"]);
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      $stockUpdateType = $updateType === "deficit" ? "Thiếu" : "Kho hàng mới";

      $event = "Stock Update ($stockUpdateType)";

      $action = $updateType === "deficit" ? "gỡ tờ" : "đã thêm đến"; //action that happened

      $eventDesc = "<p>{$qty} số lượng {$this->genmod->gettablecol('items', 'name', 'id',$itemId)} đã {$action} kho</p>
                Lý do: <p>{$desc}</p>";
      ...
      $json['msg'] = $updated ? "Đã cập nhật hàng thành công" : "Không thể cập nhật hàng tại thời điểm này. Vui lòng thử lại sau";
    } else {
      $json['status'] = 0;
      $json['msg'] = "Một hoặc nhiều trường bắt buộc trống hoặc không được điền chính xác";
  ...
  public function edit()
  {
    ...
    $this->form_validation->set_rules('itemName', 'Tên sản phẩm', [
      'required', 'trim',
      'callback_crosscheckName[' . $this->input->post('_iId', TRUE) . ']'
    ], ['required' => 'required']);
    $this->form_validation->set_rules('itemCode', 'Mã sản phẩm', [
      'required', 'trim',
      'callback_crosscheckCode[' . $this->input->post('_iId', TRUE) . ']'
    ], ['required' => 'required']);
    $this->form_validation->set_rules('itemPrice', 'Đơn giá', ['required', 'trim', 'numeric']);
    $this->form_validation->set_rules('itemDesc', 'Mô tả sản phẩm', ['trim']);
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      $this->genmod->addevent("Cập nhật sản phẩm", $itemId, $desc, 'items', $this->session->admin_id);
    } else {
      $json['status'] = 0;
      $json = $this->form_validation->error_array();
    }
  ...
  public function crosscheckName($itemName, $itemId)
  {
    ...
    if (!$itemWithName || ($itemWithName == $itemId)) {
      ...
      $this->form_validation->set_message('crosscheckName', 'Có một mục với tên này');
  ...
  public function crosscheckCode($item_code, $item_id)
  {
    ...
    if (!$item_with_code || ($item_with_code == $item_id)) {
      ...
      $this->form_validation->set_message('crosscheckCode', 'Có một mục với mã này');
