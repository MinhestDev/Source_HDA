...
class Administrators extends CI_Controller
{
  ...
  public function add()
  {
    ...
    $this->form_validation->set_rules('firstName', 'Họ', ['required', 'trim', 'max_length[20]', 'strtolower', 'ucfirst'], ['required' => "required"]);
    $this->form_validation->set_rules('lastName', 'Tên', ['required', 'trim', 'max_length[20]', 'strtolower', 'ucfirst'], ['required' => "required"]);
    $this->form_validation->set_rules(
      'email',
      'E-mail',
      ['trim', 'required', 'valid_email', 'is_unique[admin.email]', 'strtolower'],
      ['required' => "required", 'is_unique' => 'Email tồn tại']
    );
    $this->form_validation->set_rules('role', 'Phân quyền', ['required'], ['required' => "required"]);
    $this->form_validation->set_rules(
      'mobile1',
      'Số điện thoại',
      ['required', 'trim', 'numeric', 'max_length[15]', 'min_length[11]', 'is_unique[admin.mobile1]'],
      ['required' => "required", 'is_unique' => "Số này đã được đính kèm với một quản trị viên"]
    );
    $this->form_validation->set_rules('mobile2', 'Other number', ['trim', 'numeric', 'max_length[15]', 'min_length[11]']);
    $this->form_validation->set_rules('passwordOrig', 'Mật khẩu', ['required', 'min_length[8]'], ['required' => "Nhập mật khẩu"]);
    $this->form_validation->set_rules('passwordDup', 'Xác nhận mật khẩu', ['required', 'matches[passwordOrig]'], ['required' => "Hãy nhập lại mật khẩu"]);
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      $json = $inserted ?
        ['status' => 1, 'msg' => "Tài khoản admin đã được tạo thành công!"]
        :
  ...
  public function update()
  {
    ...
    $this->form_validation->set_rules('firstName', 'Họ', ['required', 'trim', 'max_length[20]'], ['required' => "required"]);
    $this->form_validation->set_rules('lastName', 'Tên', ['required', 'trim', 'max_length[20]'], ['required' => "required"]);
    $this->form_validation->set_rules('mobile1', 'Số điện thoại', [
      'required', 'trim', 'numeric', 'max_length[15]',
      'min_length[11]', 'callback_crosscheckMobile[' . $this->input->post('adminId', TRUE) . ']'
    ], ['required' => "required"]);
    $this->form_validation->set_rules('mobile2', 'Số điện thoại khác', ['trim', 'numeric', 'max_length[15]', 'min_length[11]']);
    ...
    $this->form_validation->set_rules('role', 'Phân quyền', ['required', 'trim'], ['required' => "required"]);
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      $json = $updated ?
        ['status' => 1, 'msg' => "Cập nhật thông tin admin thành công!"]
        :
        ['status' => 0, 'msg' => "Lỗi máy chủ không mong muốn! Vui lòng liên hệ với quản trị viên để được trợ giúp."];
    } else {
      //return all error messages
  ...
  public function crosscheckMobile($mobile_number, $admin_id)
  {
    ...
    if ($adminWithNum == $admin_id) {
      ...
      $this->form_validation->set_message('crosscheckMobile', 'Số này đã được đính kèm với quản trị viên');
  ...
  public function crosscheckEmail($email, $admin_id)
  {
    ...
    if ($adminWithEmail == $admin_id) {
      ...
      $this->form_validation->set_message('crosscheckEmail', 'Email này đã được đính kèm với quản trị viên');
