...
class Transactions extends CI_Controller
{
  ...
  public function nso_()
  {
    ...
    if ($allIsWell) {
      ...
      $json['msg'] = $returnedData ? "Giao dịch được xử lý thành công" :
        "Không thể xử lý yêu cầu của bạn vào lúc này. Vui lòng thử lại sau "
        . "hoặc liên hệ bộ phận kỹ thuật để được hỗ trợ";
      ...
      $eventDesc = count($arrOfItemsDetails) . " các mặt hàng tổng cộng &#8358;" . number_format($cumAmount, 0)
        . " với số tham chiếu {$returnedData['transRef']} đã được mua";

      $this->genmod->addevent("Giao dịch mới", $returnedData['transRef'], $eventDesc, 'transactions', $this->session->admin_id);
    } else { //return error msg
      $json['status'] = 0;
      $json['msg'] = "Không thể xử lý giao dịch. Hãy đảm bảo không có sai sót. Cảm ơn";
    }
